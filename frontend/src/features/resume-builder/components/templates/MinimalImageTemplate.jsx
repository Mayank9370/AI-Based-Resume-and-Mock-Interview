        ...
        <div className="max-w-5xl mx-auto bg-white text-zinc-800">
            <div className="grid grid-cols-3">
                ...
                <main className="col-span-2 p-8 pt-0">
                    ...
                    {/* DYNAMIC SECTIONS (Excluding Sidebar Items) */}
                    {data.sections && data.sections.length > 0 ? (
                        data.sections.map((section) => {
                            const { type, title, data: items } = section;
                            if (type === "skills" || type === "education" || type === "personal_info") return null;
                                                ...
                                                <div key={index}>
                                                    ...
                                                    {project.bullets && project.bullets.length > 0 && (
                                                        <ul className="list-disc list-inside text-sm text-zinc-700 mt-1">
                                                            {project.bullets.map((b, k) => b && <li key={k}>{b}</li>)}
                                                        </ul>
                                                    )}
                                                </div>
                                            ))}
                                ...
                                <section key={section.id} className="mb-8">
                                    ...
                                    <div className="space-y-4">
                                        {items.map((item, index) => (
                                            <div key={index}>
                                                <div className="flex justify-between items-center">
                                                    {(item.heading || item.title) && <h3 className="text-md font-medium text-zinc-800">{item.heading || item.title}</h3>}
                                                    {(item.date || item.start_date) && <span className="text-xs text-zinc-500">{item.date || item.start_date}</span>}
                                                </div>

                                                {(item.subheading || item.subtitle) && (
                                                    <p className="text-sm mb-1" style={{ color: accentColor }}>{item.subheading || item.subtitle}</p>
                                                )}
